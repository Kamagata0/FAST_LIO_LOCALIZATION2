...
# 相手ロボット（敵機）追跡情報
bool opponent_detected       # 敵ロボット検出成否
float64 opponent_distance    # 敵ロボットまでの距離 [m]
float64 opponent_x           # 敵ロボットのグローバルX座標 [m]
float64 opponent_y           # 敵ロボットのグローバルY座標 [m]
float64 opponent_angle_deg   # 敵ロボットの方位角 [deg]
```
